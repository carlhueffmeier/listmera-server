...
async function generatePlaylist(playlist, refresh, identifier, copy, copier) {
  ...
  if (!playlist.strict) {
    ...
    if (playlist.tracks.length <= 5) {
      ...
      seed = playlist.tracks.slice(0, 5);
    }
    const keys = [
      'dance',
      'energy',
    ...
    let recommended = await spotify
      .getRecommendations(attributes)
      .then(res => res.body.tracks.map(el => `spotify:track:${el.id}`));
    tracks = tracks.concat(recommended);
  }
  if (!copy) {
    await spotify
      .createPlaylist(playlist.adminId, playlist.name, { description: 'powered by listmera' })
      .then(res => {
        playlistId = res.body.id;
      })
      .catch(e => console.error(e));
    await spotify
      .addTracksToPlaylist(playlist.adminId, playlistId, tracks)
      .catch(e => console.error(e));
    await removeAdmin({
    ...
    await spotify
      .createPlaylist(copier.username, playlist.name, { description: 'powered by listmera' })
      .then(res => {
        playlistId = res.body.id;
      })
      .catch(e => console.error(e));
    await spotify
      .addTracksToPlaylist(copier.username, playlistId, tracks)
      .catch(e => console.error(e));
  }
}
