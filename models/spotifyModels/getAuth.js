...
async function spotifyRegister(code) {
  const newUser = {};
  let flag = false;
  await spotify.authorizationCodeGrant(code).then(async res => {
  ...
  if (flag) {
    ...
    await spotify.getUserPlaylists(newUser.username, { limit: 50 }).then(async res => {
      let completePlaylists = await Promise.all(
        res.body.items.map(async el => {
          let mappedTracks;
          await spotify
            .getPlaylistTracks(newUser.username, el.id)
            .then(res => {
              mappedTracks = res.body.items.map(el => {
                return {
                  ...
                  artists:
                    el.track.artists.length > 1
                      ? 'Various Artists'
                      : el.track.artists[0].name || 'Unknown',
            ...
            .catch(() => (mappedTracks = false));
          if (mappedTracks) {
            return { id: el.id, name: el.name, tracks: mappedTracks };
          }
        })
      ).catch(e => console.error(e));
